...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  return {
    ...
    unread,
    active,
    past: past.slice(0, PAST_ON_DASHBOARD),
    hasMorePast: past.length > PAST_ON_DASHBOARD,
  };
}

export default function Dashboard({ loaderData }: Route.ComponentProps) {
  const { haigo, orgs, unread, active, past, hasMorePast } = loaderData;
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          {hasMorePast ? (
            <Link to="/kukai" className="text-sm text-sumi-soft underline">
              すべての句会
            </Link>
          ) : null}
        </div>
